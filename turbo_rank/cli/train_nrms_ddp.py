...
from engine.ddp_train_loop import _cleanup_ddp, _setup_ddp, fit_ddp
...
def main():
    ...
    dl = DataLoader(ds, batch_size=args.batch, sampler=sampler, num_workers=4, pin_memory=True)

    # 2 Model
    model = create_model("nrms", vocab_size=vocab, embed_dim=args.embed_dim, num_heads=args.heads)
    ...
    fit_ddp(model, dl, epochs=args.epochs, lr=args.lr)
